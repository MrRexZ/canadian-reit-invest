import { CANADIANREITINVEST_PROGRAM_ADDRESS, getGreetInstruction } from '@project/anchor'
...
export function useGreetMutation({ account }: { account: UiWalletAccount }) {
  ...
  return useMutation({
    mutationFn: async () => {
      return await signAndSend(getGreetInstruction({ programAddress: CANADIANREITINVEST_PROGRAM_ADDRESS }), txSigner)
    },
    onSuccess: (signature) => {
      toastTx(signature)
    },
